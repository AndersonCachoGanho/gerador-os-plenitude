...
class GeradorOS:
    ...
    def gerar_os(self, dados, file_produto, file_vetor, nome_arquivo):
        try:
            ...
            # 1. INSERIR TEXTOS (Usando fontes STANDARD do PDF - Não dão erro)
            # 'helv' = Helvetica (Regular), 'helv-bold' = Helvetica (Negrito)
            f_bold = "helv-bold"
            f_reg = "helv"
            ...
            # 2. INSERIR FOTO DO PRODUTO (Lendo direto da memória)
            if file_produto:
                rect_prod = fitz.Rect(50, 220, 250, 450)
                page.insert_image(rect_prod, stream=file_produto.read())

            # 3. INSERIR LOGO VETORIZADO (Lendo direto da memória)
            if file_vetor:
                ...
                if file_vetor.name.lower().endswith('.pdf'):
                    ...
                    # Se for imagem comum
                    file_vetor.seek(0) # Volta ao início do arquivo
